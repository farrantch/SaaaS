...
class AWSClient(object):
    ...
    def get_rds_instances_with_stopschedule(self):
        #instances_filtered = []
        ...
        return self.filter_rds_instances_by_tag(instances, TAG_STOP_SCHEDULE)

    def get_rds_instances_with_nextscheduledstop(self):
        instances_filtered = []
        instances = self.client.describe_db_instances(
            # Filters=[
        ...
        for rdsinst in instances:
            ...
            rdsinst['Tags'] = self.client.list_tags_for_resource(ResourceName=rdsinst['DBInstanceArn'])['TagList']
            # Only include instance if available to stop
        ...
        return self.filter_rds_instances_by_tag(instances_filtered, TAG_NEXT_SCHEDULED_STOP)
    ...
    def get_instances_need_stoping(self, instances):
        instances_need_stoping = []
        for rdsinst in instances:
            rdstags = self.taglist_to_dict(rdsinst['Tags'])
            if TAG_NEXT_SCHEDULED_STOP in rdstags.keys() and len(rdstags[TAG_NEXT_SCHEDULED_STOP]) >= 1:
                # Make sure Stop Schedule tag wasn't deleted
                if TAG_STOP_SCHEDULE in rdstags.keys() and len(rdstags[TAG_STOP_SCHEDULE]) >=1:
                    if parser.parse(rdstags[TAG_NEXT_SCHEDULED_STOP]) < self.reference_time:
                        instances_need_stoping.append(rdsinst)
                # If it was deleted, remove scheduled stop tag
                else:
                    self.client.remove_tags_from_resource(
                        ResourceName=rdsinst['DBInstanceArn'],
                        TagKeys=[ TAG_NEXT_SCHEDULED_STOP ]
                    )
        return instances_need_stoping
            ...
            if TAG_STOP_SCHEDULE in rdstags.keys() and len(rdstags[TAG_STOP_SCHEDULE]) >= 1:
                try:
                    nextscheduledstop = croniter(
                        self.convert_cron_rds_syntax(rdstags[TAG_STOP_SCHEDULE]), self.reference_time).get_next(datetime)
                    # Create/Update tag if doesn't exist or is invalid
                    if TAG_NEXT_SCHEDULED_STOP not in rdstags.keys() or len(rdstags[TAG_NEXT_SCHEDULED_STOP]) < 1 or nextscheduledstop != rdstags[TAG_NEXT_SCHEDULED_STOP]:
                    ...
                    log.info("Instance {instance} has invalid cron syntax: {cron}".format(
                        instance=rdsinst['DBInstanceIdentifier'], cron=rdstags[TAG_STOP_SCHEDULE]))
...
def lambda_handler(event, context):
    ...
    assumedRoleCredentials = sts_client.assume_role(
        ...
        RoleSessionName='SaaaS_Cloud_RdsStop',
    ...
    instances_nextscheduledstop_tag = awsclient.get_rds_instances_with_nextscheduledstop()
    ...
    instances_stopschedule_tag = awsclient.get_rds_instances_with_stopschedule()
    log.info("{count} instance(s) with {tag} tag.".format(
        count=len(instances_stopschedule_tag), tag=TAG_STOP_SCHEDULE))

    # Get RDS instances that need to be stopped
    instances_need_stoping = awsclient.get_instances_need_stoping(
        instances_nextscheduledstop_tag)
    log.info("{count} instances need to be stopped.".format(
        count=len(instances_need_stoping)))

	# Stop RDS instances
    awsclient.stop_instances(instances_need_stoping, event)
